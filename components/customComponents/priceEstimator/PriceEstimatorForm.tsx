...
import { Search, Plus } from "lucide-react"; // lucide 아이콘
...
export default function PriceEstimatorForm() {
  ...
  const [result, setResult] = useState<{ wash: number; dry: number; total: number; time: number } | null>(null);

  const handleEstimate = () => {
    let washPrice = washCourse === "표준" ? 4000 : washCourse === "이불" ? 6000 : 5000;
  ...
  return (
    <div className="max-w-md mx-auto py-8 px-4 font-nanum">
      ...
      <div className="flex items-center gap-2 mb-8">
        ...
        />
        <Button variant="outline" size="icon" className="border-sky-500 text-sky-700 hover:bg-sky-100">
      ...
      <div className="mb-2 font-bold text-lg text-gray-900 flex items-center gap-2">
        ...
        <span className="text-sm text-gray-600">{useDry ? "사용" : "사용 안함"}</span>
      </div>
      <div className={`border rounded-lg p-4 mb-6 border-gray-200 bg-white shadow-sm ${!useDry ? "opacity-50" : ""}`}>
      ...
      {/* 조회 버튼 */}
      <Button className="w-full mb-6 bg-sky-600 hover:bg-sky-700 text-white font-bold text-lg" onClick={handleEstimate}>
        조회
      </Button>
