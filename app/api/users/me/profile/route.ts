...
export async function PUT(request: NextRequest) {
  try {
    const body = await request.json(); // 클라이언트에서 전송된 요청 본문(newPassword, email, phone 등)을 파싱

    // 프론트엔드 서버가 실제 백엔드 서버의 프로필 업데이트 API로 PUT 요청을 보냅니다.
    const backendResponse = await fetch(`${BACKEND_API_BASE_URL}/profile`, {
      method: "PUT",
      headers: {
      ...
      body: JSON.stringify(body), // 클라이언트로부터 받은 본문을 그대로 백엔드로 전달
      cache: "no-store", // 이 요청은 캐시하지 않도록 설정
    });

    // 백엔드로부터 받은 응답 상태를 그대로 클라이언트(브라우저)로 전달합니다.
    if (!backendResponse.ok) {
      // 백엔드에서 에러 응답을 보냈다면 해당 에러를 그대로 전달
    ...
    console.error("API 라우트 '/api/profile'에서 에러 발생:", error);
